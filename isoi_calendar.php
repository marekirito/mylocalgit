...
<div class="event_modal_edit modal hidden">
    ...
    <div class="memoBox">
        <form id="event_form_edit" method="post" action="index.php" style="padding:20px">
            ...
            <button type="button" onclick="updateEvent()">수정</button>
            <button type="button" onclick="deleteEvent()">삭제</button>
        </form>
    </div>
</div>
...
function openEventEditModal(event) {
    ...
    $('#start_date_edit').val(event.startStr);
    $('#end_date_edit').val(event.endStr);
...
function updateEvent() {
    ...
    // 구글 캘린더에도 해당 이벤트 수정
    updateGoogleCalendarEvent(eventId, title, startDate, endDate, description);
}
...
function deleteEvent() {
    if (confirm('이벤트를 삭제하시겠습니까?')) {
        var eventId = clickedEvent.id; // 클릭된 이벤트의 ID 가져옴
...
// 현재 날짜와 시간을 YYYY-MM-DDTHH:MM:SS 형식으로 변환하여 설정
$('#start_date').val(currentDate.toISOString().slice(0,16));
$('#end_date').val(currentDate.toISOString().slice(0,16));

}

    // 구글 캘린더에서 이벤트 수정
function updateGoogleCalendarEvent(eventId, title, startDate, endDate, description) {
