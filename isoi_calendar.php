                    ...
                    <?php foreach ($calendar_result as $key => $row) { ?>
                        <?php if ($row['admin_group'] == 'jasa') { ?>
                            <li>
                                ...
                                <input type="button" class="add-event-button" value="+" data-link_id="<?= $row['link_id'] ?>">
                            </li>
                        <?php } ?>
                    <?php } ?>
                </ul>
    ...
    <div class="memoBox">
        <form id="event_form" method="post" action="index.php" style="padding:20px">
            <table id="eventTable">
                ...
                <tr>
                    <th>시작일</th>
                    <td><input type="datetime-local" class="date-input" id="start_date"></td>
                </tr>
                <tr>
                    <th>종료일</th>
                    <td><input type="datetime-local" class="date-input" id="end_date"></td>
                </tr>
                ...
                <tr>
        <input type="hidden" id="calendar_id" name="calendarId" value="">
    </tr>
            </table>
...
<!-- 이벤트 수정 및 삭제 모달창 -->
<div class="event_modal_edit modal hidden">
    <div class="bg" id="event_modal_edit_bg"></div>
    <div class="memoBox">
        <form id="event_form_edit" method="post" action="index.php" style="padding:20px">
            <table id="eventTableEdit">
                <tr>
                    <th>제목</th>
                    <td><input type="text" id="event_title_edit" class="title-input"></td>
                </tr>
                <tr>
                    <th>시작일</th>
                    <td><input type="datetime-local" id="start_date_edit" class="date-input"></td>
                </tr>
                <tr>
                    <th>종료일</th>
                    <td><input type="datetime-local" id="end_date_edit" class="date-input"></td>
                </tr>
                <tr>
                    <th>설명</th>
                    <td><textarea id="description_edit" class="description-input"></textarea></td>
                </tr>
            </table>
            <div style="overflow: hidden; padding-top: 10px;">
                <button type="button" onclick="updateEvent()" style="float: left; margin-right: 10px; background-color: #87CEEB; color: #fff;">수정</button>
                <button type="button" onclick="deleteEvent()" style="float: right; margin-left: 10px; background-color: #FF6347; color: #fff;">삭제</button>
            </div>
        </form>
    </div>
</div>
...
$(document).ready(function() {
    $(document).on('click', '.add-event-button', function() {
        var calendarId = $(this).data('link_id'); // 클릭한 버튼의 data-link_id 속성을 가져옴
        $('#calendar_id').val(calendarId); // hidden input 필드에 클릭된 버튼의 data-link_id 값을 설정
        openEventModal(calendarId);
    });
});
...
// 이벤트 수정 및 삭제 모달 창 열기 함수
function openEventEditModal(event) {
    $('.event_modal_edit').removeClass('hidden');
    
    // 이벤트 정보를 모달에 표시
    $('#event_title_edit').val(event.title);
    
    // 이벤트의 시작일과 종료일을 한국 시간으로 변환하여 설정
    var startDate = new Date(event.start);
    var endDate = new Date(event.end);

    var koreaStartDate = new Date(startDate.getTime() + (9 * 60 * 60 * 1000));
    var koreaEndDate = new Date(endDate.getTime() + (9 * 60 * 60 * 1000));

    // 시작일과 종료일을 YYYY-MM-DDTHH:MM 형식으로 변환하여 설정
    var koreaStartDateTime = koreaStartDate.toISOString().slice(0, 16);
    var koreaEndDateTime = koreaEndDate.toISOString().slice(0, 16);

    $('#start_date_edit').val(koreaStartDateTime);
    $('#end_date_edit').val(koreaEndDateTime);

    $('#description_edit').val(event.extendedProps.description);
   
   // 클릭된 이벤트의 ID를 전역 변수에 저장
   clickedEvent = event;
}
...
function updateEvent() {
    ...
    // 클릭된 이벤트의 ID를 가져옴
    var eventId = clickedEvent.extendedProps.eid;
    var calendarId = clickedEvent.extendedProps.calendarId; // 클릭된 이벤트의 ID 가져옴
    ...
    var event = calendar.getEventById(eventId);
        ...
        clickedEvent.remove();
    // 모달 닫기
    $('.event_modal_edit').addClass('hidden');

    // 구글 캘린더에도 해당 이벤트 수정
    updateGoogleCalendarEvent(eventId,calendarId);
}
...
function deleteEvent() {
    if (confirm('이벤트를 삭제하시겠습니까?')) {
        var eventId = clickedEvent.extendedProps.eid; // 클릭된 이벤트의 ID 가져옴
        var calendarId = clickedEvent.extendedProps.calendarId; // 클릭된 이벤트의 ID 가져옴
...
//배경클릭시 모달창닫기
    ...
    $('#event_modal_edit_bg').on("click", function(){
        $('.event_modal_edit').addClass('hidden');
        $("#event_form")[0].reset();
    }
    )
// 이벤트 모달 열기 함수 정의
function openEventModal(calendarId) {
    // 모달 열기
    ...
    // GMT에서 한국 시간으로 변환
    var koreaTime = new Date(currentDate.getTime() + (9 * 60 * 60 * 1000));

    // 한국 시간으로 설정된 시작일과 종료일 설정 (YYYY-MM-DDTHH:MM 형식)
    var koreaStartTime = koreaTime.toISOString().slice(0, 16);
    var koreaEndTime = koreaTime.toISOString().slice(0, 16);

  // 종료일에는 최소한 일분을 추가하여 설정
  var koreaEndTimeWithoneminute = new Date(koreaTime.getTime() + (1 * 60 * 1000));
    var koreaEndTimeWithoneminuteFormatted = koreaEndTimeWithoneminute.toISOString().slice(0, 16);

    // 입력 필드에 값 설정
    $('#start_date').val(koreaStartTime);
    $('#end_date').val(koreaEndTimeWithoneminuteFormatted);

}

    // 구글 캘린더에서 이벤트 수정
function updateGoogleCalendarEvent(eventId,calendarId) {
    // AJAX를 사용하여 서버로 데이터 전송하고, 서버에서 구글 캘린더 API를 사용하여 이벤트 수정 요청을 보냄
    var title = $('#event_title_edit').val();
    var startDate = $('#start_date_edit').val();
    var endDate = $('#end_date_edit').val();
    var description = $('#description_edit').val();
    $.ajax({
        url: "update_google_calendar_event.php", // 이벤트 수정을 처리하는 서버 측 파일 경로
        type: "POST",
        data: {
            'eventId': eventId,
            'title': title,
        ...
        success: function(response) {
            if (response.success) {
                console.log('구글 캘린더에서 이벤트 수정 성공');
            ...
            calendar.addEvent(event);
                ...
                console.error('구글 캘린더에서 이벤트 수정 중 오류 발생');
            }
        },
...
// 구글 캘린더에서 이벤트 삭제
function deleteGoogleCalendarEvent(eventId,calendarId) {
    // AJAX를 사용하여 서버로 데이터 전송하고, 서버에서 구글 캘린더 API를 사용하여 이벤트 삭제 요청을 보냄
    $.ajax({
        url: "delete_google_calendar_event.php", // 이벤트 삭제를 처리하는 서버 측 파일 경로
        type: "POST",
        data: {
            'eventId': eventId,
            calendarId: calendarId
        ...
        success: function(response) {
            if (response.success) {
                console.log('구글 캘린더에서 이벤트 삭제 성공');
            } else {  
                alert('이벤트 삭제 중 오류가 발생했습니다.');
                console.error('구글 캘린더에서 이벤트 삭제 중 오류 발생');
            }
        },
        error: function(xhr, status, error) {
            alert('이벤트 삭제 중 오류가 발생했습니다.');
